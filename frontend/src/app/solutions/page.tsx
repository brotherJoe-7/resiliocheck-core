    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', color: 'var(--text-primary)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '80px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 72 }}>
          <div style={{ display: 'inline-block', fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1.5px', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 16 }}>Solutions</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {solutions.map((s) => (
            <div key={s.tag} className="rc-grid-2" style={{ gap: 40, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '40px 28px', alignItems: 'center' }}>
              <div>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(234,88,12,0.1)', border: '1px solid rgba(234,88,12,0.3)', borderRadius: 6, padding: '4px 12px', marginBottom: 20 }}>
                  ...
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--accent)' }}>{s.tag}</span>
                </div>
                <h2 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: 12, marginTop: 0 }}>{s.headline}</h2>
                <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: 1.8, marginTop: 0 }}>{s.desc}</p>
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 16 }}>Includes</div>
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {s.points.map(p => (
                    <li key={p} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.9rem', color: '#e4e4e7' }}>
                      <Check size={16} color="var(--accent)" /> {p}
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div style={{ textAlign: 'center', marginTop: 72 }}>
          <Link href="/" style={{ background: 'var(--accent)', color: 'var(--text-primary)', borderRadius: 8, padding: '14px 32px', textDecoration: 'none', fontWeight: 700, fontSize: '1rem' }}>
            See Pricing →
          </Link>
        </div>
      </div>
